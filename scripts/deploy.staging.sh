...
~/.local/bin/aws s3 cp dist/example.html s3://stage.knowgod.com --acl public-read --cache-control 'no-cache'
~/.local/bin/aws s3 sync dist/mobile s3://stage.knowgod.com --acl public-read --cache-control 'no-cache' --content-type 'application/json'
