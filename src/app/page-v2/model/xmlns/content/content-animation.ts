...
export class KgwContentAnimation {
    ...
    parse(): KgwContentComplexTypeAnimation {
        ...
        if (this._xmlNode.getAttribute('autoplay')) {
            item.attributes.autoplay = this._xmlNode.getAttribute('autoplay') === 'true';
        }
        ...
        if (this._xmlNode.getAttribute('play-listeners')) {
            item.attributes.playListeners = this._xmlNode.getAttribute('play-listeners');
        }
        if (this._xmlNode.getAttribute('stop-listeners')) {
            item.attributes.stopListeners = this._xmlNode.getAttribute('stop-listeners');
        }
